...
function Dashboard() {
    ...
    const [newUser, setNewUser] = useState({ name: '', Activity: '', Duration: '', Date: '', Actions: '' });
    ...
    const addUser = async (e) => {
        e.preventDefault();
        if (!newUser.name || !newUser.Activity || !newUser.Duration || !newUser.Date || !newUser.Actions) return;

        try {
            const res = await axios.post('http://localhost:8086/api/', newUser);
            setUser([...user, res.data]);
            setNewUser({ name: '', Activity: '', Duration: '', Date: '', Actions: '' });
        } catch (error) {
            console.log(error);
        }
    };
